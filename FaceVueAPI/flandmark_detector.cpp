...
void flandmark_argmax(double *smax, double *value,FLANDMARK_Options *options, const int *mapTable, FLANDMARK_PSI_SPARSE *Psi_sparse, double **q, double **g)
{
    uint8_t M = options->M;
...
int flandmark_detect_base(uint8_t* face_image,double* value ,FLANDMARK_Model* model, double * landmarks)
{
    ...
    // argmax
    flandmark_argmax(landmarks, value, &model->data.options, mapTable, Psi_sparse, q, g);
...
int flandmark_detect(IplImage *img, double* value, int *bbox ,FLANDMARK_Model *model, double *landmarks, int *bw_margin)
{
    int retval = 0;
    if (bw_margin)
	{
    ...
    retval = flandmark_detect_base(model->normalizedImageFrame, value, model, landmarks);
    if (retval)
    {
